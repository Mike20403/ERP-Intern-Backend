...
namespace DotNetStarter.Queries.Users.Get
{
    public sealed class GetUserHandler : BaseRequestHandler<GetUser, User>
    {
        ...
        {
            return await _unitOfWork.UserRepository.FindAsync($"{ClassUtils.GetPropertyName<User>(u => u.Role)},{ClassUtils.GetPropertyName<User>(u => u.Privileges)}", u => u.Id == request.UserId);
        }
    }
}
